...
tape("write_buf callback function validation", function (t) {
  ...
  fuse.ops.write_buf = function (path, fd, buf, offset, cb) {
    ...
    try {
      process.nextTick(cb, buf.length); // Return bytes written
    } catch (err) {
  ...
  const mockSignal = (bytesWritten) => {
    t.equal(bytesWritten, 9, "write_buf should return bytes written");
...
tape("write_buf undefined callback handling", function (t) {
  ...
  try {
    ...
    t.fail(
      "_op_write_buf should not throw when operation not implemented: " +
        err.message,
    );
  }
});
...
tape("read_buf undefined callback handling", function (t) {
  ...
  try {
    ...
    t.fail(
      "_op_read_buf should not throw when operation not implemented: " +
        err.message,
    );
  }
});
...
tape("write_buf parameter count validation", function (t) {
  ...
  fuse.ops.write_buf = function () {
    ...
    t.equal(
      typeof callback,
      "function",
      "last argument should always be the callback function",
    );
    ...
    callback(args[2].length); // Return bytes written
  };

  const mockSignal = (bytesWritten) => {
    t.equal(bytesWritten, 4, "should receive bytes written");
    t.end();
  };
