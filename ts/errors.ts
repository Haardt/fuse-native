 ...
 */
export const ERRNO_CODES: Record<number, string> = {
  [-1]: 'EPERM',
  [-2]: 'ENOENT',
  [-3]: 'ESRCH',
  [-4]: 'EINTR',
 ...
 */
export const CODE_TO_ERRNO: Record<string, number> = Object.fromEntries(
  Object.entries(ERRNO_CODES).map(([errno, code]) => [code, parseInt(errno)])
);

/**
 * Human-readable error messages for common errno codes
 */
export const ERRNO_MESSAGES: Record<number, string> = {
 ...
 */
export function getErrnoFromCode(code: string): number {
 ...
 */
export function getCodeFromErrno(errno: number): string {
 ...
 */
export function isFuseError(error: unknown): error is FuseErrno {
 ...
 */
export function toFuseError(error: unknown, fallbackErrno = -5): FuseErrno {
 ...
 */
export const createENoEnt = (message?: string, path?: string) =>
  new FuseErrno('ENOENT', message, 'stat', path);

export const createEAcces = (message?: string, path?: string) =>
  new FuseErrno('EACCES', message, 'open', path);

export const createEExist = (message?: string, path?: string) =>
  new FuseErrno('EEXIST', message, 'open', path);

export const createEIsDir = (message?: string, path?: string) =>
  new FuseErrno('EISDIR', message, 'read', path);

export const createENotDir = (message?: string, path?: string) =>
  new FuseErrno('ENOTDIR', message, 'opendir', path);

export const createENoTEmpty = (message?: string, path?: string) =>
  new FuseErrno('ENOTEMPTY', message, 'rmdir', path);

export const createEPerm = (message?: string, path?: string) =>
  new FuseErrno('EPERM', message, 'chmod', path);

export const createEIO = (message?: string, path?: string) =>
  new FuseErrno('EIO', message, 'read', path);

export const createEInval = (message?: string) =>
  new FuseErrno('EINVAL', message);

export const createENoSpc = (message?: string, path?: string) =>
  new FuseErrno('ENOSPC', message, 'write', path);

/**
 * Type guard for checking specific errno codes
 */
export function isErrno(error: unknown, code: string): error is FuseErrno {
...
export class ErrorHandler {
  ...
  static wrapAsync<T extends any[], R>(
    fn: (...args: T) => Promise<R>,
    errorTransform?: (error: unknown) => FuseErrno
  ) {
