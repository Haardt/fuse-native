...
class Fuse extends Nanoresource {
  constructor(mnt, ops, opts = {}) {
    ...
    if (!implemented.includes(binding.op_copy_file_range))
      implemented.push(binding.op_copy_file_range);
  ...
  _fuseOptions() {
    ...
    if (/\*|(^,)fuse-bindings(,$)/.test(process.env.DEBUG) || this.opts.debug)
      options.push("debug");
  ...
  _op_read_buf(signal, path, bufp, len, offsetLow, offsetHigh, fd) {
    const offset = getDoubleArg(offsetLow, offsetHigh);
    if (!this.ops.read_buf) {
      return signal(Fuse.ENOSYS);
    }
    this.ops.read_buf(path, fd, bufp, len, offset, (err) => {
      return signal(err);
    });
  }
  ...
  _op_copy_file_range(
    ...
    let posIn = offsetIn >>> 0;
    let posOut = offsetOut >>> 0;
    let total = 0;
    const max = len >>> 0;
