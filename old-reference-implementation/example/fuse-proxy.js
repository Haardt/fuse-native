...
const ops = {
  ...
  write_buf: function (path, fd, buf, pos, cb) {
    console.log(
      `write_buf: ${path} (fd: ${fd}, buf.length: ${buf ? buf.length : "null"}, pos: ${pos})`,
    );

    // Validate input parameters to prevent infinite loops
    if (!buf || !Buffer.isBuffer(buf) || buf.length === 0) {
      console.log(`write_buf: Invalid buffer for ${path}`);
      return cb(-22); // EINVAL
    }

    if (pos < 0) {
      console.log(`write_buf: Invalid position ${pos} for ${path}`);
      return cb(-22); // EINVAL
    }

    fs.write(fd, buf, 0, buf.length, pos, (err, bytesWritten) => {
      if (err) {
        console.log(`write_buf: Error writing ${path}: ${err.message}`);
        return cb(err.errno || -err.code || -5);
      }

      // Ensure we return a valid byte count
      const validBytesWritten = Math.max(
        0,
        Math.min(bytesWritten || 0, buf.length),
      );
      console.log(
        `write_buf: Successfully wrote ${validBytesWritten} bytes to ${path}`,
      );
  ...
  read_buf: function (path, fd, bufp, len, pos, cb) {
    ...
    if (!bufp || !Buffer.isBuffer(bufp)) {
      console.log(`read_buf: Invalid buffer pointer for ${path}`);
      return cb(-22); // EINVAL
    }

    if (len < 0 || len > bufp.length) {
      console.log(
        `read_buf: Invalid length ${len} for ${path} (buffer size: ${bufp.length})`,
      );
      return cb(-22); // EINVAL
    }

    if (pos < 0) {
      console.log(`read_buf: Invalid position ${pos} for ${path}`);
      return cb(-22); // EINVAL
    }
    ...
    fs.read(fd, buf, 0, len, pos, (err, bytesRead) => {
      if (err) {
        console.log(`read_buf: Error reading ${path}: ${err.message}`);
        return cb(err.errno || -err.code || -5);
      }
      ...
      try {
        if (validBytesRead > 0) {
          // Only copy actual bytes read to prevent buffer overflow
          bufp.set(buf.slice(0, validBytesRead), 0);
        }
        console.log(
          `read_buf: Successfully read ${validBytesRead} bytes from ${path}`,
        );
        cb(0, validBytesRead);
      } catch (bufferErr) {
        console.log(
          `read_buf: Buffer operation error for ${path}: ${bufferErr.message}`,
        );
        cb(-5); // EIO
      }
    });
...
fuse.mount((err) => {
  ...
  console.log(
    "This will take a few minutes as it downloads and installs packages.",
  );
  console.log("");
