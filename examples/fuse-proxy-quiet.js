...
function logOperation(op, path, details = '') {
  ...
  if (op === 'create' || op === 'mkdir' || (op === 'write_buf' && details.includes('package.json'))) {
...
const ops = {
  ...
  write_buf: function (path, fd, buf, pos, cb) {
    if (path.includes('package.json')) {
      logOperation('write_buf', path, `(${buf.length} bytes)`)
    }
...
let statsInterval = setInterval(() => {
  console.log(`📊 Operations: ${stats.operations}, Files: ${stats.files_created}, Dirs: ${stats.dirs_created}, Written: ${(stats.bytes_written/1024/1024).toFixed(1)}MB, Read: ${(stats.bytes_read/1024/1024).toFixed(1)}MB`)
}, 10000)
...
fuse.mount((err) => {
  ...
  child.stdout.on('data', (data) => {
    ...
    for (const line of lines) {
      if (line.includes('Creating') || line.includes('Installing') ||
          line.includes('Success') || line.includes('Happy') ||
          line.includes('npm') || line.includes('yarn') ||
          line.includes('Done') || line.includes('compiled')) {
  ...
  child.on('close', (code) => {
    ...
    console.log(`   • Data written: ${(stats.bytes_written/1024/1024).toFixed(1)}MB`)
    console.log(`   • Data read: ${(stats.bytes_read/1024/1024).toFixed(1)}MB`)
    ...
    if (code === 0) {
      ...
      try {
        ...
        if (hasNodeModules) {
          ...
          console.log(`   📦 node_modules/ contains ${nodeModulesCount} packages`)
        }
        ...
        console.log(`   • File creation and writing (${stats.files_created} files)`)
        console.log(`   • Directory creation (${stats.dirs_created} directories)`)
        ...
        console.log(``)
        console.log(`🔄 All operations were transparently forwarded to: ${TARGET_DIR}`)
    ...
    console.log(``)
    console.log(`🎯 FUSE proxy is still running and ready for more operations!`)
    console.log(``)
